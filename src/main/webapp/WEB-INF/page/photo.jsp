    ...
    <script type="text/javascript">
        ...
        function init() {
            ...
            var gridColumns = [[
                ...
                {field: 'auditInfo', title: '审核备注', hidden: true},
        ...
        // 审核不通过的方法
        function checkStateNo() {
            var photoId = $("#photoId").val();
            if (!photoId) {
                $.messager.alert("温馨提示", "操作失败：未获取到图片ID！", "warning");
                return true;
            }
            var auditInfo = $('#auditInfo').textbox('getValue');
            if (!auditInfo) {
                $.messager.alert("温馨提示", "审核不通过请填写审核备注！", "warning");
                return true;
            }
            var url = contentUrl + "/photo/auditNo";
            $.messager.progress();
            var data = {photoId: photoId, auditInfo: auditInfo};
            Util.request(url, data, function (result) {
        ...
        function nextPhoto() {
            ...
            Util.request(contentUrl + "/photo/next", {photoId: photoId, nextState: 1}, function (result) {
                if (result.success) {
                    $("#auditForm").form("reset");
...
<div id="auditDialog" class="easyui-dialog" style="width:800px; height: 600px; padding: 0px"
    ...
    <div class="easyui-layout" style="width: 100%; height: 100%;">
        ...
        <div data-options="region:'east',iconCls:'icon-edit'" style="width:320px;padding:10px">
